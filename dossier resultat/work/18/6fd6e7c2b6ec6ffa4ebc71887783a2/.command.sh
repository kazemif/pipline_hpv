#!/bin/bash -ue
echo "🔍 Traitement du fichier BAM pour l'échantillon : barcode16"

# Vérifie si le fichier BAM existe
if [ ! -s barcode16.sorted.bam ]; then
    echo "❌ Fichier BAM vide : barcode16.sorted.bam"
...
if [ -z "$CSV_FILE" ]; then
    ...
    echo -e "sample_id\tstatus\tnote" > alignment_metrics_barcode16.txt
    echo -e "barcode16\tvide\tPas de métriques disponibles" >> alignment_metrics_barcode16.txt
else
