...
READ_COUNT=$(cat output_barcode03/barcode03_read_count.txt 2>/dev/null || echo "NA")
GC_CONTENT=$(cat output_barcode03/barcode03_GC_content.txt 2>/dev/null || echo "NA")
READ_LENGTH=$(cat output_barcode03/barcode03_read_length.txt 2>/dev/null || echo "NA")
MAP_SCORE=$(cat output_barcode03/barcode03_map_score.txt 2>/dev/null || echo "NA")
SEQ_SCORE=$(cat output_barcode03/barcode03_seq_score.txt 2>/dev/null || echo "NA")

echo -e "sample_id	read_count	gc_content	seq_score	read_length	map_score" > stat_barcode03.tsv
echo -e "barcode03	$READ_COUNT	$GC_CONTENT	$SEQ_SCORE	$READ_LENGTH	$MAP_SCORE" >> stat_barcode03.tsv
