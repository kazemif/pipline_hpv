#!/bin/bash -ue
mkdir -p barcode01_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode01.sorted.bam > barcode01_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode01.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode01_read_alignment_stats/per_read_stats.tsv
