#!/bin/bash -ue
mkdir -p barcode02_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode02.sorted.bam > barcode02_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode02.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode02_read_alignment_stats/per_read_stats.tsv
