...
READ_COUNT=$(cat output_barcode15/barcode15_read_count.txt 2>/dev/null || echo "NA")
GC_CONTENT=$(cat output_barcode15/barcode15_GC_content.txt 2>/dev/null || echo "NA")
READ_LENGTH=$(cat output_barcode15/barcode15_read_length.txt 2>/dev/null || echo "NA")
MAP_SCORE=$(cat output_barcode15/barcode15_map_score.txt 2>/dev/null || echo "NA")
SEQ_SCORE=$(cat output_barcode15/barcode15_seq_score.txt 2>/dev/null || echo "NA")

echo -e "sample_id	read_count	gc_content	seq_score	read_length	map_score" > stat_barcode15.tsv
echo -e "barcode15	$READ_COUNT	$GC_CONTENT	$SEQ_SCORE	$READ_LENGTH	$MAP_SCORE" >> stat_barcode15.tsv
