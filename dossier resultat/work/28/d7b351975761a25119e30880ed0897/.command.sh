#!/bin/bash -ue
mkdir -p barcode09_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode09.sorted.bam > barcode09_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode09.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode09_read_alignment_stats/per_read_stats.tsv
