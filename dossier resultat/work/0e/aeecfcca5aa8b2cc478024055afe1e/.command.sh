#!/bin/bash -ue
set -e

# Étape 1 : Générer la séquence consensus à partir du VCF
bcftools consensus -f sequence.fasta barcode07.sorted.vcf.gz > barcode07.tmp.fa

# Étape 2 : Filtrer le fichier BED (important pour éviter les erreurs)
awk '$2 < $3' barcode07.coverage.bed > barcode07.lowcov.valid.bed

# Étape 3 : Masquer les régions à faible couverture
bedtools maskfasta -fi barcode07.tmp.fa -bed barcode07.lowcov.valid.bed -fo barcode07.consensus.fa
