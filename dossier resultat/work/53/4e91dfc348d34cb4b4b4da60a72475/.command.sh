#!/bin/bash -ue
mkdir -p barcode07_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode07.sorted.bam > barcode07_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode07.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode07_read_alignment_stats/per_read_stats.tsv
