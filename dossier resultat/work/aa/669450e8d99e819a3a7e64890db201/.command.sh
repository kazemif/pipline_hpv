#!/bin/bash -ue
mkdir -p barcode15_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode15.sorted.bam > barcode15_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode15.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode15_read_alignment_stats/per_read_stats.tsv
