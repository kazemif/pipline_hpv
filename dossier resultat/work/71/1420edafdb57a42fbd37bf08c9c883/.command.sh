#!/bin/bash -ue
set -e

# Étape 1 : Générer la séquence consensus à partir du VCF
bcftools consensus -f sequence.fasta barcode16.sorted.vcf.gz > barcode16.tmp.fa

# Étape 2 : Filtrer le fichier BED (important pour éviter les erreurs)
awk '$2 < $3' barcode16.coverage.bed > barcode16.lowcov.valid.bed

# Étape 3 : Masquer les régions à faible couverture
bedtools maskfasta -fi barcode16.tmp.fa -bed barcode16.lowcov.valid.bed -fo barcode16.consensus.fa
