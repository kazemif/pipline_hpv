#!/bin/bash -ue
mkdir -p barcode04_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode04.sorted.bam > barcode04_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode04.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode04_read_alignment_stats/per_read_stats.tsv
