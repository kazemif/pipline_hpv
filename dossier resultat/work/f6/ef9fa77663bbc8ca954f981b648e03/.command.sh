#!/bin/bash -ue
mkdir -p barcode11_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode11.sorted.bam > barcode11_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode11.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode11_read_alignment_stats/per_read_stats.tsv
