...
#Function to collect provided command line arguments----------------------------
def get_arguments():
    """
    ...
    parser = argparse.ArgumentParser(prog = "parse_fastq.py",
                                     description = "This program allows to extract \
                                        read length and sequencing quality score average \
                                        for each reads")
...
def parse_arguments(args):
    ...
    if args.input is None:
        sys.exit("ERROR: You must provide a fastq/fastq.gz input file")
    ...
    if file_extension not in ["fastq", "fastq.gz"]:
