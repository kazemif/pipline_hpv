...
echo ""
echo "Welcome in parse read align ! This tool will allow you to parse and analyse SAM/BAM file"
...
#Install parse read align  environment=========================================
if [[ $(conda env list | egrep "parse_read_align" | wc -l) != 1 ]]
then
    echo ""
    echo -e "${YELLOW}MESSAGE:${RESET} Installing suitable environment"
    mamba env create -f ../../env/parse_read_align.yml
fi
echo ""
echo -e "${GREEN}CHECK:${RESET} parse read align environment is installed"
...
echo ""
echo -e "${GREEN}CHECK:${RESET} parse read tool is installed"