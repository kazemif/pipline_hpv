#!/bin/bash -ue
mkdir -p barcode03_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode03.sorted.bam > barcode03_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode03.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode03_read_alignment_stats/per_read_stats.tsv
