#!/bin/bash -ue
mkdir -p barcode16_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode16.sorted.bam > barcode16_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode16.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode16_read_alignment_stats/per_read_stats.tsv
