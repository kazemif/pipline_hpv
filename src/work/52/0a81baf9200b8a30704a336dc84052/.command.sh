#!/bin/bash -ue
mkdir -p barcode08_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode08.sorted.bam > barcode08_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode08.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode08_read_alignment_stats/per_read_stats.tsv
