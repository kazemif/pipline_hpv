#!/bin/bash -ue
mkdir -p barcode05_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode05.sorted.bam > barcode05_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode05.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode05_read_alignment_stats/per_read_stats.tsv
