#!/bin/bash -ue
mkdir -p barcode12_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode12.sorted.bam > barcode12_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode12.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode12_read_alignment_stats/per_read_stats.tsv
