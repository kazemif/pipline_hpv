#!/bin/bash -ue
mkdir -p barcode10_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode10.sorted.bam > barcode10_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode10.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode10_read_alignment_stats/per_read_stats.tsv
