#!/bin/bash -ue
mkdir -p barcode14_read_alignment_stats

# 1. Nombre de reads mappés et unmappés
samtools flagstat barcode14.sorted.bam > barcode14_read_alignment_stats/mapped_unmapped.txt

# 2. Statistiques par read
samtools view barcode14.sorted.bam | awk '
BEGIN {
    OFS="\t"
...
' > barcode14_read_alignment_stats/per_read_stats.tsv
